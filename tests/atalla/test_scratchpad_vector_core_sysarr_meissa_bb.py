...
class GSAUMEISSABridge(Clocked):
    ...
    def _pack_rsp(self, out_row: List[float], meta: Dict) -> Dict:
        ...
        for i, val in enumerate(out_row[: self.sa.p]):
            vec[i] = float(val)
    ...
    def tick(self, time: Optional[float] = None) -> None:
        ...
        if req is not None:
            ...
            if bool(req.get("is_weight", False)):
                row = vdata[: self.sa.p]
                if len(row) < self.sa.p:
                    row += [0.0] * (self.sa.p - len(row))
            else:
                row = vdata[: self.sa.m]
                if len(row) < self.sa.m:
                    row += [0.0] * (self.sa.m - len(row))

            assert self.sa.issue(
                {
                    "vdata": row,
                    "is_weight": bool(req.get("is_weight", False)),
        ...
        while self.sa.has_response():
            ...
            if rsp is None:
                break
            assert self.vc.push_systolic_response(self._pack_rsp(list(rsp["vdata"]), dict(rsp.get("meta", {}))))


def test_scratchpad_vector_core_sysarr_meissa_bb_end_to_end():
    log_dir = Path(__file__).resolve().parents[2] / "logs" / "sysarr_gemm_meissa_bb"
    ...
    try:
        ...
        sa = SystolicArrayMEISSABlackbox(n=tile, m=tile, p=tile)

        vls_bridge = VLSFrontendBridge(vc, spad, vls_id=0, frontend_id=0)
        sysarr_bridge = GSAUMEISSABridge(vc, sa)
        ...
        expected = _matmul_u16(act, wgt)

        _write_dram_tile_u16(dram, DRAM_ACT, act)
        _write_dram_tile_u16(dram, DRAM_WGT, wgt)
        ...
        if len(state["completed_rows"]) < tile:
            raise AssertionError("timed out waiting for DRAM->SPAD->VRF->GSAU->MEISSA->VRF->SPAD->DRAM")
        ...
        if got != expected:
            ...
            dprintf("SYSARR", f"exp[{mismatch_row}][:8]={expected[mismatch_row][:8]}")
            dprintf("SYSARR", f"got[0][:8]={got[0][:8]}")
            dprintf("SYSARR", f"exp[0][:8]={expected[0][:8]}")
            dprintf("SYSARR", f"got[-1][:8]={got[-1][:8]}")
            dprintf("SYSARR", f"exp[-1][:8]={expected[-1][:8]}")
        assert got == expected
    finally:
        close_debug()
...
if __name__ == "__main__":
    test_scratchpad_vector_core_sysarr_meissa_bb_end_to_end()
