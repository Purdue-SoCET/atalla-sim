...
def plot_gsau_rd_queue(rows: List[Dict[str, Any]], output_path: Path) -> Path:
    ...
    avg_depth = [float((row.get("queue_avg_depths") or {}).get("gsau_rd_queue", 0.0)) for row in rows]

    fig, ax = plt.subplots(figsize=(9.5, 5.5))
    ax.plot(x, max_depth, marker="o", linewidth=2.4, color="#e15759", label="max gsau rd queue")
    ax.plot(x, avg_depth, marker="s", linewidth=2.0, linestyle="--", color="#59a14f", label="avg gsau rd queue")
...
def plot_summary(rows: List[Dict[str, Any]], output_path: Path) -> Path:
    ...
    axes[1].plot(x, avg_depth, marker="s", linewidth=2.0, linestyle="--", color="#59a14f", label="avg")
