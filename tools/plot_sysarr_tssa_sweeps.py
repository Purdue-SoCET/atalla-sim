...
def plot_sweep(rows: List[Dict[str, str]], sweep_name: str, output_dir: Path) -> Path:
    ...
    axes[0, 1].set_title("Phase Breakdown")
    ...
    ax_util = axes[1, 0]
    ax_tp = ax_util.twinx()
    ax_util.plot(x, [_num(row["mac_utilization"]) for row in ordered], marker="o", color="#54a24b", label="mac_utilization")
    ax_tp.plot(
        x,
    ...
    ax_util.set_title("Utilization And Throughput")
    ax_util.set_xlabel(ordered[0]["param_name"])
    ax_util.set_ylabel("mac utilization")
